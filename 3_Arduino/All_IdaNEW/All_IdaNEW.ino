...
const int waterPumpPin = 21;  // ปั๊มน้ำ (Relay)
...
unsigned long pumpDuration = 5000;  // ระยะเวลาเปิดปั๊ม (มิลลิวินาที) ค่าเริ่มต้น 5 วินาที
enum PumpWaitState { NOT_WAITING, WAITING_BEFORE_PUMP, PUMPING };
PumpWaitState pumpWaitState = NOT_WAITING;
unsigned long pumpWaitStartTime = 0;
const long PUMP_WAIT_TIME = 1000;  // รอ 1 วินาที ก่อนเปิดปั๊ม
...
// === WATER PUMP CONTROL ===
void startPump() {
  digitalWrite(waterPumpPin, HIGH);  // เปิด Relay
  pumpRunning = true;
...
void stopPump() {
  digitalWrite(waterPumpPin, LOW);  // ปิด Relay
  pumpRunning = false;
  Serial.println("Pump: OFF");
}

void handlePumpTimer() {
  // ตรวจสอบว่าปั๊มทำงานเกินเวลาที่กำหนดหรือไม่
  if (pumpRunning == true) {
...
void handleTimedSequence() {
  ...
  if (timedSequenceStep == 1) {
    setTargetPosition(1);
    timedSequenceStep = 2;
    pumpWaitState = NOT_WAITING;  // เตรียมพร้อมสำหรับรอบถัดไป
  ...
  if (timedSequenceStep > 5) {
    ...
    pumpWaitState = NOT_WAITING;
    return;
  }

  // จัดการรอ + ปั๊มน้ำ
  if (pumpWaitState == NOT_WAITING) {
    // ถ้าครบเวลารอแล้ว ให้เริ่มรอ 1 วินาที ก่อนเปิดปั๊ม
    if (millis() - timedSequenceWaitStartTime >= TIMED_SEQUENCE_WAIT_TIME) {
      pumpWaitState = WAITING_BEFORE_PUMP;
      pumpWaitStartTime = millis();
      Serial.println("Waiting 1 sec before pump...");
    }
  }
  else if (pumpWaitState == WAITING_BEFORE_PUMP) {
    // รอ 1 วินาที แล้วเปิดปั๊ม
    if (millis() - pumpWaitStartTime >= PUMP_WAIT_TIME) {
      startPump();
      pumpWaitState = PUMPING;
      Serial.println("Pumping water...");
    }
  }
  else if (pumpWaitState == PUMPING) {
    // รอให้ปั๊มทำงานเสร็จ (handlePumpTimer จะปิดให้)
    if (pumpRunning == false) {
      // ปั๊มปิดแล้ว ไปตำแหน่งถัดไป
...
void processSerialCommand(String cmd) {
  ...
  if (cmd.startsWith("COLOR:")) {
    ...
    String pumpCmd = cmd.substring(5);

    if (pumpCmd == "ON") {
      // เปิดปั๊มด้วยตนเอง (Manual mode)
      if (currentMode == MANUAL) {
        startPump();
      ...
      Serial.println("ERROR:INVALID_PUMP_COMMAND");
    }
  }
  else if (cmd.startsWith("PUMP_DURATION:")) {
    // ตั้งเวลาปั๊มน้ำ (มิลลิวินาที)
    long duration = cmd.substring(14).toInt();
    if (duration > 0) {
      pumpDuration = duration;
      Serial.print("ACK:PUMP_DURATION_SET:");
...
void setup() {
  ...
  digitalWrite(waterPumpPin, LOW);  // เริ่มต้นปิดปั๊ม
...
void loop() {
  ...
  handleTimedSequence();
  handlePumpTimer();  // ตรวจสอบเวลาปั๊มน้ำ
  processColorData();
